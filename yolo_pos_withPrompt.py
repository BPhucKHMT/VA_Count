...
def process_images(
    text_file_path, dataset_path, yolo_model, preprocess, output_folder, device="cpu"
):
    boxes_dict = {}
    with open(text_file_path, "r") as f:
        for line in f:
            image_name, class_name = line.strip().split("\t")
            print(f"Processing image: {image_name}, prompt: {class_name}")
            ...
            for i, (box, conf) in enumerate(zip(boxes, confs)):
                ...
                if (
                    patch.size == (0, 0)
                    or not is_valid_patch(patch, binary_classifier, preprocess, device)
                    or x2 - x1 > w / 2
                    or y2 - y1 > h / 2
                    or y2 - y1 < 5
                    or x2 - x1 < 5
...
def main(args):
    ...
    text_file_path = os.path.join(
        args.root_path, "ImageClasses_FSC147_detailed_v6_pos.txt"
    )
    ...
    output_json_path = os.path.join(
        args.root_path, "annotation_FSC147_pos_yolo_v2.json"
    )
    ...
    for image_name, boxes in boxes_dict.items():
        if image_name in data:
            new_boxes = [
                [[x1, y1], [x1, y2], [x2, y2], [x2, y1]] for x1, y1, x2, y2 in boxes
            ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Root path to the dataset and output files",
    )
